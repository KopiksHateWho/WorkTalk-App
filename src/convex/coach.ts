...
const SYSTEM_PROMPT = `You are the WorkTalk Quest coach, a warm English speaking tutor for students who are preparing for their careers and for English competitions.
...
async function callProvider(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `[WorkTalk Quest coach] provider responded ${response.status}: ${await response
          .text()
          .catch(() => "")}`.slice(0, 400),
      );
      return null;
    ...
    console.warn("[WorkTalk Quest coach] provider call failed", error);
    return null;
  } finally {
